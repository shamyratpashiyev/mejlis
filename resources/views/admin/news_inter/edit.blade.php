...
<div class="content">
    ...
    <form action="{{ route('news_inter.update', $selected_news->id) }}" method="POST" class="edit__form" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <!-- ROW  -->
        <div class="row">
        ...
        <div class="row">
            ...
            <div class="form__item w15">
                ...
                <input type="datetime-local" class="inputDate" name="event_date" value="{{ date('Y-m-d',strtotime($selected_news->event_date)) }}">
                @error('date')
                    <p class="err">{{ $message }}</p>
                @enderror
            </div>
