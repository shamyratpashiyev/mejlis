@extends('layout', ['title' => __('app.articles_page.title')])
...
@php
    $links_list = [
                    ...
                    ['name'=>__('app.articles_page.title'), 'is_active'=>'active', 'url' => route('articles_page',['lang' => app()->getLocale()])], 
    ...
    $breadcrumbs_array = [
                    ...
                    ['name' => __('app.articles_page.title'), 'url' => request()->fullUrl()],
    ];
@endphp
        ...
        <div class="inner_wrapper flex_column">
            ...
            <div class="page_content_block flex_row">
                ...
                <div class="articles_block">
                    ...
                    <h3 class="block_title">@lang('app.articles_page.title')</h3>

                    <div class="articles_row flex_row">
                        @foreach ($articles_current_page as $article)
                            <div data-href-value="{{ route('single_article_page',['id'=>$article->id,'lang'=>app()->getLocale()]) }}" class="article_container flex_column">
                                <div class="top_block flex_column">
                                    ...
                                    <h3 class="title">{{ $article->{'title_' . $current_lang->code} }}</h3>
                                    <div class="content">
                                        {!! $article->{'description_' . $current_lang->code} !!}
                                    </div>
                                </div>

                                <div class="bottom_row flex_row">
                                    <span class="date">{{ date('d.m.Y', strtotime($article->published_date)) }}</span>
                                    <a href="{{ route('single_article_page',['id'=>$article->id,'lang'=>app()->getLocale()]) }}" class="flex_row hover_underline">
            ...
            <div class="pagination_row flex_row">
                <a class="flex_row" style="align-items: center;" href="{{ route('articles_page', ['page_num' => $prev_page, 'lang' => app()->getLocale()]) }}">
                    <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                @foreach ($pagination_array as $page)
                    ...
                    @else
                        <a href="{{ route('articles_page', ['page_num' => $page, 'lang' => app()->getLocale()]) }}" class="item @if($current_page == $page) active @endif">{{ $page }}</a>
                    @endif
                @endforeach

                <a href="{{ route('articles_page', ['page_num' => $next_page, 'lang' => app()->getLocale()]) }}">
                    <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
